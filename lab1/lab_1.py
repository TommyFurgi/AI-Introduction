...
# %%
df = df.loc[~df["Neighborhood"].isin(["GrnHill", "Landmrk"]), :] # usowa rekordy z tymi dzielnicami
...
# df.loc zachowuje te wiersze które spełniają warunek ponady na pierwszym miejscu
...
# %% editable=true slideshow={"slide_type": ""} tags=["ex"]
# your_code
replace_na(df, "CentralAir", value="No") # klimatyzacja
replace_na(df, "EnclosedPorch", value=0) # rozmiar ganka
replace_na(df, "Fireplaces", value=0) # liczba kominów
replace_na(df, "SaleCondition", value="Normal") # warunek sprzedaży 
...
y = df.pop("SalePrice") # usuwa kolumne z df

categorical_features = df.select_dtypes(include="object").columns # kolumny kategoryczne
numerical_features = df.select_dtypes(exclude="object").columns # kolumny tekstowe

X_train, X_test, y_train, y_test = train_test_split(
    df, y, test_size=0.3, random_state=0
) # losowo dzieli na zbiór testowy (random_state jest swojego rodzaju seedem by dla tych samych dane podział wyglądał tak samo)
...
reg_linear = LinearRegression(fit_intercept=False) # robimy regresje
reg_linear.fit(X_train, y_train) # dopasowywujemy do wyników treningowych

y_pred = reg_linear.predict(X_test) # na podstawie regresji tworzymy predykcje wyników testowych
rmse = root_mean_squared_error(y_test, y_pred) # porównujemy predykcje z wynikwami danych testowych
...
def assess_regression_model(model, X_train, X_test, y_train, y_test) -> tuple[float, float]:
    ...
    train_pred = model.predict(X_train) # robimy predykcje wyników
    test_pred = model.predict(X_test)

    # exponential transform for y_train, y_test and predictions
    train_pred_exp = np.expm1(train_pred) # odwrotna transformacja logarytmiczna (używamy funkcji wykładniczej)
    ...
    rmse_train = root_mean_squared_error(y_train_exp, train_pred_exp) # obliczamy błędy
    rmse_test = root_mean_squared_error(y_test_exp, test_pred_exp)
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Różnica sugeruje, że model radzi sobie lepiej na danych treningowych niż na terstowych. Różnica to około 5000 co może wskazywać na **przeuczenie** modelu. 
...
reg_ridge = Ridge(random_state=0) # regularyzacja L2
reg_lasso = Lasso(random_state=0) # regularyzacja L1

reg_ridge.fit(X_train, y_train) # standaryzacja
...
alphas_ridge = np.linspace(0.1, 100, 1000) # 1000 wartości
ridge_model = RidgeCV(alphas=alphas_ridge, cv=None) # tworzymy model - cv=None to use LOOCV
ridge_model.fit(X_train, y_train)

lasso_model = LassoCV(n_alphas=1000, cv=5, random_state=0) # cv=5 -> 5-fold
lasso_model.fit(X_train, y_train)

reg_ridge_alpha = round(ridge_model.alpha_, 4) # hiperparametr
reg_lasso_alpha = round(lasso_model.alpha_, 4)
print(f"Siła regularyzacji Ridge: {reg_ridge_alpha}")
print(f"Siła regularyzacji Lasso: {reg_lasso_alpha}")

print("\nRidge: ")
ridge_train_rmse, ridge_test_rmse = assess_regression_model(ridge_model, X_train, X_test, y_train, y_test) 

print("\nLasso: ")
lasso_train_rmse, lasso_test_rmse = assess_regression_model(lasso_model, X_train, X_test, y_train, y_test) 
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Tak udało sie wyeliminować overfitting używająć regularyzacji L1 (model lasso) , RSME obu zbiorów mają bardzo zbliżone wartości. Jeśli chodzi o model Ridge to wyniki są trochę lepsze. Za pomocą regularyzacji z tuningiem hierparametrów udało nam się wyeliminować przeuczenie modelu, który wystempował w regresji liniowej bez regularyzacji.
...
replace_na(df, "education", value="unknown") # zamieniamy puste pola na "unknown"

y = df.pop("y") # usuwa kolumne z df
...
# trzeba było uzupełnić kolumne 'education', ale zrobiłem to w zadaniu 7 bo bez tego pojawiał sie błąd przy sprawdzaniu poprawności
...
plt.xlabel("Klasy")
plt.ylabel("Częstość")
plt.xticks(ticks=range(2), labels=["No", "Yes"], rotation=45)
plt.show()
...
model_l0 = LogisticRegression(penalty=None, class_weight="balanced").fit(X_train, y_train)
y_pred_l0 = model_l0.predict(X_test)

model_l2 = LogisticRegressionCV(Cs=100, cv=5, scoring="f1", class_weight="balanced", random_state=0, n_jobs=-1).fit(X_train, y_train)
y_pred_l2 = model_l2.predict(X_test)

model_l1 = LogisticRegressionCV(Cs=100, cv=5, scoring="f1", solver="saga", class_weight="balanced", random_state=0, n_jobs=-1).fit(X_train, y_train)
y_pred_l1 = model_l1.predict(X_test)

l0_precision, l0_recall, l0_f1 = precision_score(y_test, y_pred_l0), recall_score(y_test, y_pred_l0), f1_score(y_test, y_pred_l0)
l1_precision, l1_recall, l1_f1 = precision_score(y_test, y_pred_l1), recall_score(y_test, y_pred_l1), f1_score(y_test, y_pred_l1)
l2_precision, l2_recall, l2_f1 = precision_score(y_test, y_pred_l2), recall_score(y_test, y_pred_l2), f1_score(y_test, y_pred_l2)

print(f"Level 0 - Precision: {l0_precision:.4f}, Recall: {l0_recall:.4f}, F1 Score: {l0_f1:.4f}")
print(f"Level 1 - Precision: {l1_precision:.4f}, Recall: {l1_recall:.4f}, F1 Score: {l1_f1:.4f}")
print(f"Level 2 - Precision: {l2_precision:.4f}, Recall: {l2_recall:.4f}, F1 Score: {l2_f1:.4f}")
...
# %% editable=true slideshow={"slide_type": ""} tags=["ex"]
# your_code
y_train_pred = model_l0.predict(X_train)
f1_train = f1_score(y_train, y_train_pred)

y_test_pred = model_l0.predict(X_test)
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Tworzenie modeli z regularyzacją w tym przypadku nie ma sensu bo czułość, precyzja i miaram F1 mają takie same wartości bez wzgledu na to czy stosujemy regularyzajce L1, L2 czy nie stosujemy jej wcale. 
#
# Dla naszego problemu ważniejsza jest **prezycja**, ponieważ interesują nas skuteczny marketing bez zbędnych kosztów. By zmienić studunek czułość:precyzja można by dostosować próg klasyfikacji, czyli zmienić wartość, powyżej której model uznaje przypadek za pozytywny. Aczkolwiek **czułość** również jest dla nas istotna bo intereseje nas by nasz marketing dotarł to jak największej liczby osób. Nie można mieć jednego i drugiego dlatego warto brać pod uwagę metrykę F1 ktróra jest balansem pomiędzy precyzją, a czułością.
...
logistic_pipeline = Pipeline([
    ...
    ("classifier", LogisticRegression(penalty=None, class_weight="balanced", max_iter=1000, random_state=0))
])
...
print(f"F1 Score na zbiorze treningowym: {f1_train*100:2f}%")
print(f"F1 Score na zbiorze testowym: {f1_test*100:.2f}%")
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
#
# Nie ma sensu tworzyć modelu z regularyzacją, ponieważ wagi modelu są już stosunkowo małe zatem nie występuje zjawisko nadmiernego dopasowania (overfitting). Wynika to ze zbliżonych wyników F1 na zbiorze treningowym i testowym. Dalszy tuning hiperparametrów zwiększyłby koszty obliczeniowe bez gwarancji istotnej poprawy jakości modelu.
#
